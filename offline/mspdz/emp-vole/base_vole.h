#ifndef BASE_VOLE_NEW_H__
#define BASE_VOLE_NEW_H__

#include "emp-tool/emp-tool.h"
#include "emp-zk/emp-vole/cope.h"

template <typename IO> class Base_vole {
public:
  int party;
  int m;
  IO *io;
  ...
  // SENDER
  Base_vole(int party, IO *io, __uint128_t Delta) {
    this->party = party;
    this->io = io;
  ...
  // RECEIVER
  Base_vole(int party, IO *io, uint64_t *x) {
    this->party = party;
    this->io = io;
    this->x = x;
    cope = new Cope<IO>(party, io, MERSENNE_PRIME_EXP);
