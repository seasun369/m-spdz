#ifndef _VOLE_TRIPLE_N_H_
#define _VOLE_TRIPLE_N_H_
#include "base_vole.h"
...
*/
template <typename IO> class VoleTriple_N {
public:
  IO *io;
  IO **ios;
  int party;
  ...
  VoleTriple_N(int party, int threads, IO **ios, uint64_t *x,
  ...
  void setup() {
    ...
    if (party == ALICE) {
      ...
      vole0 = new Base_vole<IO>(party, ios[0], Delta);
      vole0->triple_gen_send(key, triple_n0);

      extend_send(pre_yz0, &mpfss_pre0, &pre_ot_ini0, &lpn_pre0, key);
      delete[] key;
    } else {
      __uint128_t *mac = new __uint128_t[triple_n0];
      vole0 = new Base_vole<IO>(party, ios[0], x);
      vole0->triple_gen_recv(mac, triple_n0);
