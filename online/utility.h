...
//
#else
inline uint64_t* transform(uint64_t* A, int matrix_sz){
  uint64_t *A_T = new uint64_t[matrix_sz * matrix_sz];
  for(int i=0; i < matrix_sz; ++i){
    for(int j =0; j < matrix_sz; ++j){
      uint64_t tmp = A[i*matrix_sz+j];
...
inline void mult_mod_bch4(block *res, block *a, uint64_t *b) {
  ...
  __m512i t1 = bs[0] & PR8;
  __m512i t2 = _mm512_srli_epi64(bs[0], MERSENNE_PRIME_EXP) ^
               _mm512_slli_epi64(bs[1], 64 - MERSENNE_PRIME_EXP);
...
inline void mult_mod_bch2(block *res, block *a, uint64_t *b) {
  ...
  t1[0] = bs[0] & prs;
  t1[1] = bs[1] & prs;
  t2[0] = _mm_srli_epi64(bs[0], MERSENNE_PRIME_EXP) ^
          _mm_slli_epi64(bs[2], 64 - MERSENNE_PRIME_EXP);
  t2[1] = _mm_srli_epi64(bs[1], MERSENNE_PRIME_EXP) ^
          _mm_slli_epi64(bs[3], 64 - MERSENNE_PRIME_EXP);
...
inline void mult_mod_bch2(uint64_t *res, uint64_t *a, uint64_t *b) {
  ...
  eb = _mm_add_epi64(eb & prs, _mm_srli_epi64(eb, MERSENNE_PRIME_EXP) ^
                                   _mm_slli_epi64(cb, 64 - MERSENNE_PRIME_EXP));
...
inline void mult_mod_bch4(uint64_t *res, uint64_t *a, uint64_t *b) {
  ...
  resb[0] = _mm_add_epi64((eb[0] & prs),
                          (_mm_srli_epi64(eb[0], MERSENNE_PRIME_EXP) ^
                           _mm_slli_epi64(cb[0], 64 - MERSENNE_PRIME_EXP)));
  resb[1] = _mm_add_epi64((eb[1] & prs),
                          (_mm_srli_epi64(eb[1], MERSENNE_PRIME_EXP) ^
                           _mm_slli_epi64(cb[1], 64 - MERSENNE_PRIME_EXP)));
...
*/
inline uint64_t* mat_mult_mod(uint64_t* a, uint64_t* b, int mat_sz){
  uint64_t * res = new uint64_t[mat_sz * mat_sz];
  for (int i = 0; i < mat_sz; ++i) {
            for (int j = 0; j < mat_sz; ++j) {
                for (int k = 0; k < mat_sz; ++k) {
                    uint64_t tmp = mult_mod(a[i * mat_sz + j] , b[j * mat_sz + k]);
                    res[i * mat_sz + k] = add_mod(res[i * mat_sz + k] , tmp);
                }
            }
        } 
  return res;
