...
public:
	...
	void Online_mul(mpz_t *x, mpz_t *y, mpz_t *mac_x, mpz_t *mac_y, mpz_t *output, mpz_t *output_mac)
	{
		...
		mpz_t *ae = new mpz_t[sz];
		mpz_t *de = new mpz_t[sz];
		mpz_t *mac_db = new mpz_t[sz];
		mpz_t *mac_ae = new mpz_t[sz];
		mpz_t *mac_de = new mpz_t[sz];
		...
		for(int i=0;i<mat_sz;i++){
			for(int j=0;j<mat_sz;j++){
				...
				for(int k=0;k<mat_sz;k++){
					mpz_mul(multc1[index1],d[i*mat_sz+k],b[k*mat_sz+j]);
					mpz_add(db[index1],db[index1],multc1[index1]);
					mpz_mul(multc1[index1],a[i*mat_sz+k],e[k*mat_sz+j]);
					mpz_add(ae[index1],ae[index1],multc1[index1]);
					mpz_mul(multc1[index1],d[i*mat_sz+k],e[k*mat_sz+j]);
					mpz_add(de[index1],de[index1],multc1[index1]);
					mpz_mul(multc1[index1],d[i*mat_sz+k],mac_b[k*mat_sz+j]);
					mpz_add(mac_db[index1],mac_db[index1],multc1[index1]);
					mpz_mul(multc1[index1],mac_a[i*mat_sz+k],e[k*mat_sz+j]);
					mpz_add(mac_ae[index1],mac_ae[index1],multc1[index1]);
					mpz_mul(mac_de[index1],de[index1],key);
				}
				mpz_mod(db[index1],db[index1],ppp);
				mpz_mod(ae[index1],ae[index1],ppp);
				mpz_mod(de[index1],de[index1],ppp);
				mpz_mod(mac_db[index1],mac_db[index1],ppp);
				mpz_mod(mac_ae[index1],mac_ae[index1],ppp);
				mpz_mod(mac_de[index1],mac_de[index1],ppp);
			}
		}
		...
		for (int i = 0; i < sz; ++i)
		{
			...
			mpz_add(output_mac[i],output_mac[i],mac_ae[i]);
	...
	void mac_check(mpz_t *x, mpz_t *mac_x)
	{
		...
		#pragma omp parallel for
		for (int i = 0; i < sz; i++){
			mpz_mul(kk[i],x[i],key);
			mpz_sub(sigma_x[i],mac_x[i],kk[i]);
