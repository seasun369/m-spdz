...
public:
	...
	void Online_mul(uint64_t* x, uint64_t* y, uint64_t* mac_x, uint64_t* mac_y, uint64_t* output, uint64_t* output_mac) {
		...
		for (int i = 0; i < sz; ++i) {
			if (party == 1) {
				output[i] = c[i] + db[i] + ae[i] + de[i];
			}
			else {
				output[i] = c[i] + db[i] + ae[i];
			}
			output_mac[i] = mac_c[i] + mac_db[i] + mac_ae[i] + mac_de[i];
		}
	...
	void mac_check(uint64_t* x, uint64_t* mac_x) {
		...
		if (party == 1) {
			for (int i = 0; i < sz; i++) sigma_x[i] = X[i] * key;
		}
		else {
			for (int i = 0; i < sz; i++) sigma_x[i] = x[i] * key;
		}

		for (int i = 0; i < sz; ++i) sigma_x[i] = mac_x[i] - sigma_x[i];
		...
		for (int i = 0; i < mat_sz; ++i) {
			for (int j = 0; j < mat_sz; ++j) {
				for (int k = 0; k < mat_sz; ++k) {
					res[i * mat_sz + k] = res[i * mat_sz + k] + a[i * mat_sz + j] * b[j * mat_sz + k];
				}
			}
		}
		return res;
