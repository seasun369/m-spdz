#ifndef EMP_BROADCAST_IO_CHANNEL
#define EMP_BROADCAST_IO_CHANNEL
...
class BoradcastNetIO: public IOChannel<BoradcastNetIO> { public:
	bool is_server;
	int mysocket = -1;
	int consocket = -1;
	FILE * stream = nullptr;
	...
	string addr;
	int port;
    BoradcastNetIO(const char * address, int port, bool quiet = false) {
		...
		if (address == nullptr) {
			...
			int reuse = 1;
			setsockopt(mysocket, SOL_SOCKET, SO_BROADCAST, (const char*)&reuse, sizeof(reuse));
			...
			while(1) {
				consocket = socket(AF_INET, SO_BROADCAST, 0);
...
#endif  //NETWORK_BROADCAST_IO_CHANNEL